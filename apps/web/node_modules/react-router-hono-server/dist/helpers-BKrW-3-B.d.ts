...
import { ServerBuild, UNSAFE_MiddlewareEnabled, unstable_RouterContext, AppLoadContext } from 'react-router';

type ReactRouterHonoServerAppLoadContext = UNSAFE_MiddlewareEnabled extends true
  ? Map<unstable_RouterContext, unknown>
  : AppLoadContext;

interface HonoServerOptionsBase<E extends Env> {
  /**
...
interface WithWebsocket<E extends Env> {
  /**
...
interface WithoutWebsocket<E extends Env> {
  /**
   * Enable WebSockets support in `configure`
   *
   * For `bun` and `cloudflare` we will use the `@hono/node-ws`'s `injectWebSocket` on dev (only),
 ...
 */
declare function createGetLoadContext(getLoadContext: HonoServerOptionsBase<Env>["getLoadContext"]): ((c: hono.Context<Env, any, {}>, options: {
    build: ServerBuild;
    mode: string;
}) => Promise<ReactRouterHonoServerAppLoadContext> | ReactRouterHonoServerAppLoadContext) | undefined;

export { type HonoServerOptionsBase as H, type WithoutWebsocket as W, type WithWebsocket as a, createGetLoadContext as c };
